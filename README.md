...
- The React Query hooks call `apiFetch` (see `src/lib/apiClient.ts`). Set `NEXT_PUBLIC_API_BASE_URL` to point at a staging/production API and flip `NEXT_PUBLIC_USE_MOCKS=false`; normal `/api/*` traffic uses the built-in proxy, while managed-evidence uploads and binary downloads use `apiFetchDirect` to avoid the hosting function's smaller payload ceiling. Mock-mode file traffic remains on the local route.
...
2. Toggle `NEXT_PUBLIC_USE_MOCKS` to `false` to bypass the mock handlers. Normal `/api/*` requests proxy to the backend; managed-evidence uploads and binary downloads go directly to the same base URL.
