...
- A download embeds every managed evidence file as an original-file PDF attachment after checking its byte count and SHA-256. Valid PDF, JPEG, and PNG exhibits also receive visible preview pages; other formats remain attachments and are not converted. Generation stops for a missing/mismatched file or when managed exhibits exceed the 100 MB packet limit. Legacy metadata-only references remain visible in the manifest but have no bytes to embed unless they match a managed file from the arbitration.
